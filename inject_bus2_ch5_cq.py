"""
Bus2 Chapter 5 CQ Injection Pipeline
=====================================
Parses BUS_2_CH5_CQ - Sheet1.csv and injects into testDatabase.bus2["5"]
Also cleans up pre-existing "Board Board" occurrences in Chapter 4.
"""
import csv
import json
import re
...
CSV_PATH = 'BUS_2_CH5_CQ - Sheet1.csv'
DB_PATH = 'data.js'
BACKUP_PATH = 'data.js.bak_bus2_ch5'
...
def inject_into_database(cqs):
    """Inject chapter 5 CQ data and clean up Chapter 4 'Board Board' patterns."""
    ...
    # Locate Chapter 4 closing brace
    ch4_idx = content.find('"Chapter 4 : Organizing"')
    if ch4_idx == -1:
        print("  [FAIL] Could not find Chapter 4 in data.js")
        return False
    
    ch4_block_start = content.rfind('{', 0, ch4_idx)
    bracket = 0
    ch4_block_end = -1
    for i in range(ch4_block_start, len(content)):
        if content[i] == '{':
            bracket += 1
        elif content[i] == '}':
            bracket -= 1
    ...
    if ch4_block_end == -1:
        print("  [FAIL] Could not trace Chapter 4 block end")
        return False
        
    print(f"  Chapter 4 block ends at index {ch4_block_end}")
    
    # Build Chapter 5 object
    chapter5_data = {
        "subjectName": "Business Organization & Management 2nd Paper",
        "chapterName": "Chapter 5 : Staffing",
        "mcqData": [],
        "shortCQData": [],
        "fullCQData": cqs
    }
    
    ch5_json = json.dumps(chapter5_data, indent=8, ensure_ascii=False)
    
    # Insert new Chapter 5 at ch4_block_end
    insert_text = ',\n    "5": ' + ch5_json
    new_content = content[:ch4_block_end] + insert_text + content[ch4_block_end:]
    
    # Clean up duplicate "Board Board" in Chapter 4 section to ensure perfect data quality
    # We will locate the Chapter 4 section and replace '"meta": "X Board Board \u00b7 Y"' with '"meta": "X Board \u00b7 Y"'
    # Or simply replace ' Board Board ' with ' Board ' across the whole data.js to be absolutely robust!
    ch4_after_idx = new_content.find('"Chapter 4 : Organizing"')
    if ch4_after_idx != -1:
        # Let's target replacements of 'Board Board' in the vicinity of Chapter 4 and Chapter 5
        # Since 'Board Board' should never exist anywhere in data.js, a global replacement is extremely safe.
        count_bb = new_content.count("Board Board")
        if count_bb > 0:
            new_content = new_content.replace("Board Board \u00b7", "Board \u00b7")
            new_content = new_content.replace("Board Board \u00b7", "Board \u00b7") # Handle any possible variation
            print(f"  [OK] Cleaned up {count_bb} duplicate 'Board Board' occurrences in the database!")
        else:
            print("  [INFO] No pre-existing 'Board Board' duplicates found")
            
    with open(DB_PATH, 'w', encoding='utf-8') as f:
        f.write(new_content)
        
    print(f"  [OK] Injected Chapter 5 and cleaned Chapter 4 successfully")
    return True
...
def post_validate():
    ...
    # 6. Count CQs in Chapter 5
    ch5_marker = '"Chapter 5 : Staffing"'
    if ch5_marker in content:
        ch5_start = content.find(ch5_marker)
        ch5_section = content[ch5_start:ch5_start + 700000]
        id_count = len(re.findall(r'"id"\s*:\s*"', ch5_section))
        print(f"  CQ count in Chapter 5 section: {id_count}")
...
if __name__ == '__main__':
    print("=" * 60)
    print("Bus2 Chapter 5 CQ Injection Pipeline")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print(f"SUCCESS: {len(cqs)} CQs injected into testDatabase.bus2['5']")
    print("=" * 60)
