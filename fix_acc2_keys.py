...
def fix_injection():
    ...
    # Prepare the acc2 structure with CORRECT keys: shortCQData and fullCQData
    acc2_data = {
        "4": {
    ...
    # Find the acc2 section I added previously and replace it
    # My previous script added "acc2": { ... }
    start_marker = '"acc2": {'
    start_idx = content.find(start_marker)
    
    if start_idx != -1:
        # Find the end of the acc2 block
        # It ends before "ict": {
        end_marker = '"ict": {'
        end_idx = content.find(end_marker, start_idx)
        
        if end_idx != -1:
            # We need to preserve the trailing comma and spaces
            # The previous injection was: acc2_js + ",\n\n    " + content[idx:]
            # So we should find the comma after acc2 block
            
            # Use brace counting to find the exact end of acc2 object
            brace_depth = 0
            ...
            if block_end != -1:
                ...
                with open('data.js', 'w', encoding='utf-8') as f:
                    f.write(new_content)
                print("Fixed acc2 injection with fullCQData key")
            else:
                print("Could not find end of acc2 block")
        else:
            print("Could not find ict block")
    else:
