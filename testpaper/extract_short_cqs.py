...
def process():
    ...
    # Find bus1 and bus2 subjects
    for subj in ['"bus1":', '"bus2":']:
        subj_idx = content.find(subj)
        if subj_idx == -1: continue

        # The subject block ends when the matching bracket closes
        bracket_count = 0
        ...
        for ch_match in re.finditer(r'("\d+":\s*\{)', subj_block):
            ...
            full_start = full_match.end() - 1 # points to '['
            arr_count = 0
            full_end = -1
                ...
                if full_cq_str[i] == '{':
                    ...
                    if cq_depth == 0 and cq_start != -1:
                        ...
                        if q_arr_match:
                            ...
                            # Using regex to find label a and b safely
                            # Allow multiple spaces and newlines
        ...
        # We need to update content dynamically since the offsets change
        # Wait, modifying `content` inside the loop where we are iterating over static offsets?
        # `subj_idx`, `subj_start`, `subj_end` are from the original string.
        # But we only do two subjects, and we replace them sequentially. 
        # Actually it's safer to do replacing one by one and updating content each time.
        pass

# Since we need to update content safely:
import sys
def safe_process():
    with open('data.js', 'r', encoding='utf-8') as f:
        content = f.read()
