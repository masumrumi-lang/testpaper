...
def clean_html(text):
    if not text:
        return ""
    # Convert newlines to <br> for the stem and simple text
    text = text.replace('\n', '<br>')
    return text.strip()
...
def process_cq_csv(file_path):
    ...
    # We use newline='' for csv reader to handle multiline cells correctly
    with open(file_path, mode='r', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            ...
            # 2. Short CQ Structure (Knowledge & Comprehension)
            # We'll add Question A and B as separate short CQs
...
if __name__ == "__main__":
    input_file = "Business Org & Mgmt 1 CQ - Sheet1.csv"
    data = process_cq_csv(input_file)
    
    with open("bus1_cq_data.json", "w", encoding="utf-8") as f:
