...
const csvPath = 'c:\\Users\\BMTF\\.antigravity\\testpaper\\Fin2_all_ch_mcq - Sheet1.csv';
...
function parseCSV(content) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        // Also capture the raw array to support index-based column recovery for shifted rows
        obj._rawRow = r;
...
function run() {
    ...
    // 3. Extract fin2 block
    const startIdx = dbContent.indexOf('"fin2":');
    if (startIdx === -1) {
        throw new Error("Could not find 'fin2' block in data.js");
    }
    ...
    if (endIdx === -1) {
        throw new Error("Could not find matching closing brace for 'fin2'");
    }

    const fin2BlockStr = dbContent.slice(startIdx, endIdx);
    const jsonStr = fin2BlockStr.split(':').slice(1).join(':').trim();
    
    // Evaluate fin2 block to JS object
    const fin2Data = (new Function(`return ${jsonStr}`))();
    console.log("Successfully parsed fin2 block from data.js.");
    ...
    rawMcqs.forEach((row, index) => {
        ...
        const chStr = row['Chapter'] || '';
        const ch = parseInt(chStr, 10);
        if (isNaN(ch) || ch < 1 || ch > 14) {
            console.warn(`Row ${rowNum}: Invalid chapter "${row['Chapter']}". Skipping.`);
            return;
        }

        let questionText = row['Question'] || '';
        let optA = row['Option A'] || '';
        let optB = row['Option B'] || '';
        let optC = row['Option C'] || '';
        let optD = row['Option D'] || '';
        let correctAnsRaw = row['Correct Answer'] || '';
        let explanationText = row['Explanation'] || '';

        // 1. Shift Type A (Row 255 & 256)
        if (rowNum === 255 || rowNum === 256) {
            optD = "i, ii & iii";
            correctAnsRaw = row['Option D']; // holds correct answer key 'c' or 'b'
            explanationText = row['Correct Answer']; // holds explanation text
        }
        
        // 2. Shift Type B (Row 582 to 590)
        else if (rowNum >= 582 && rowNum <= 590) {
            const raw = row._rawRow;
            const qPart1 = raw[5] || '';
            const qPart2 = raw[6] || '';
            questionText = (qPart1 === 'N/A' || qPart1 === '') ? qPart2 : `${qPart1}\n\n${qPart2}`;
            optA = raw[7] || '';
            optB = raw[8] || '';
            optC = raw[9] || '';
            optD = raw[10] || '';
            correctAnsRaw = raw[11] || '';
            explanationText = raw[12] || '';
        }
        
        // 3. Special Case for Row 870
        else if (rowNum === 870) {
            optD = "None of the above";
            correctAnsRaw = "a";
            explanationText = row['Option D']; // holds explanation text
        }

        // Normalize Question Text
        const question = parseRomanQuestion(questionText);

        // Normalize Correct Answer Key
        let ansChar = correctAnsRaw.toLowerCase().trim();
        if (ansChar.startsWith('option ')) {
            ansChar = ansChar.replace('option ', '').trim();
        }
        
        const correctAnswer = ansMap[ansChar];
        if (correctAnswer === undefined) {
            throw new Error(`Row ${rowNum}: Invalid Correct Answer "${correctAnsRaw}" (normalized: "${ansChar}")`);
        }
        ...
        let explanation = '';
        if (explanationText) {
            explanation = wrapInP(explanationText.trim());
        }
    ...
    for (let ch = 1; ch <= 14; ch++) {
        ...
        const originalCount = fin2Data[ch.toString()].mcqData ? fin2Data[ch.toString()].mcqData.length : 0;
        ...
        fin2Data[ch.toString()].mcqData = mcqs;
    ...
    // Re-serialize the fin2 block
    const serializedFin2 = JSON.stringify(fin2Data, null, 4);
    
    // Replace in dbContent
    const updatedDbContent = dbContent.slice(0, startIdx) + `"fin2": ` + serializedFin2 + dbContent.slice(endIdx);
