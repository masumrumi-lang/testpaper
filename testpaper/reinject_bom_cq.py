...
def re_inject_cqs():
    ...
    with open(csv_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            # FIXED REGEX: Match the first number found in the chapter string
            ch_match = re.search(r'(\d+)', chapter_str)
            if ch_match:
                ch_id = str(int(ch_match.group(1))) # Convert '03' to '3'
            else:
                ch_id = "1"
    ...
    # 1. Clear existing fullCQData from ALL chapters in bus1 to undo the mistake
    bus1_content = re.sub(r'("fullCQData"\s*:\s*\[.*?\])', '"fullCQData": []', bus1_content, flags=re.DOTALL)
    
    # 2. Inject the correct data for each chapter
    for ch_id, cq_list in bom_data.items():
        ...
        if ch_match:
            ...
            # Since we cleared it to [], we can just safely replace it
            new_ch_content = re.sub(r'"fullCQData"\s*:\s*\[\]', f'"fullCQData": {cq_json}', ch_content)
    ...
    print(f"Injected fullCQData successfully into {len(bom_data)} chapters!")
    for k in sorted(bom_data.keys()):
        print(f"Chapter {k}: {len(bom_data[k])} CQs")
