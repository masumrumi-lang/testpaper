...
def get_svg():
    ...
    angles_line = [-90, 0, 45, 90, 135, 180, 225]
    angles_mid = [-45, 22.5, 67.5, 112.5, 157.5, 202.5, 247.5]
    
    svg = f"""<p class="mb-4 text-gray-700 dark:text-gray-300">Observe the cycle -</p>
<div class="flex justify-center w-full my-6 flex-col items-center">
<svg viewBox="-150 -150 300 300" width="100%" style="max-width: 350px; font-family: sans-serif;" xmlns="http://www.w3.org/2000/svg">
  <!-- Outer Circle -->
  ...
  <g stroke="#1f2937" stroke-width="3">\n"""
    ...
    # Texts
    labels = ["Planning", "A", "Staffing", "Directing", "Motivating", "B", "Controlling"]
    ...
    for i in range(7):
        ...
        font_size = "12" if len(labels[i]) > 7 else "14"
        if labels[i] in ["A", "B"]:
            font_size = "20"
            font_weight = "bold"
        elif labels[i] == "Planning":
            font_size = "15"
            font_weight = "600"
        else:
            font_weight = "600"
...
def process():
    ...
    if array_start != -1:
        ...
        print("Successfully updated CQ 09 with English SVG and text replacement.")
    else:
