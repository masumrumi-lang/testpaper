...
# We format it to look like the existing data.js
...
# Insertion point: after the last subject before 'ict' or before the final };
insert_idx = -1
...
# 5. Safety Validation (Dry Run)
temp_path = data_js_path + '.temp'
with open(temp_path, 'w', encoding='utf-8') as f:
    f.writelines(new_lines)

print("Validating syntax with node -c...")
try:
    result = subprocess.run(['node', '-c', temp_path], capture_output=True, text=True, check=True)
